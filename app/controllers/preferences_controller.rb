class PreferencesController < ApplicationController
  ...
  def overview
    if params[:query].present?
      ...
      @cuisines = Tag.where(category: 'cuisines').sort_by{|tag| -tag.preferences.count }.first(4)
      @types = Tag.where(category: 'dish_types').sort_by{|tag| -tag.preferences.count }.first(4)

      @ingredients = Ingredient.all.sort_by{|ing| -ing.preferences.count }.first(4)
  end
